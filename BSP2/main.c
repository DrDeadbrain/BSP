 ...
 */
void *
philoThread (void *input_arguments)
{
  ...
  while (!(keyInp == 'q'))
	...
	{
	  put_weights (&weights,
		       thread_info_init (args, current_State, weights));
	}
      else if (current_State == GET_WEIGHTS)
	{
	  get_weights (args.weightNeeded, &weights,
		       thread_info_init (args, current_State, weights));
	}
    }
  args.status = 'q';
 ...
 */
static int
updateState (State_p * state, long *counter_l)
{
  int hasNew = 0;
  switch (*state)
    {
 ...
 */
void *
thread_dependencies (void *input_arguments)
{
  Thread_arguments args = *((Thread_arguments *) input_arguments);
 ...
 */
Thread_info
thread_info_init (Thread_arguments currArgs, State_p currState,
		  Weights_s currStack)
{
  Thread_info outInfo;
 ...
 */
char
convertForOutput (State_p state)
{
  switch (state)
    {
    case WORKOUT:
 ...
 */
void
writeInput (int threadID, char value)
{
  if (!(threadID >= 0 && threadID < PHIL_T))
    {
      printf ("Thread ID is invalid!\n");
...
main (int argv, char *args[])
{
  ...
  input_v[0] = (Thread_arguments)
  {
  .ID = 0,.name = "Anna",.weightNeeded = ANNA_W, 'n', &semaphore[0]};
  input_v[1] = (Thread_arguments)
  {
  .ID = 1,.name = "Bernd",.weightNeeded = BERND_W, 'n', &semaphore[1]};
  input_v[2] = (Thread_arguments)
  {
  .ID = 2,.name = "Clara",.weightNeeded = CLARA_W, 'n', &semaphore[2]};
  input_v[3] = (Thread_arguments)
  {
  .ID = 3,.name = "Dirk",.weightNeeded = DIRK_W, 'n', &semaphore[3]};
  input_v[4] = (Thread_arguments)
  {
  .ID = 4,.name = "Emma",.weightNeeded = EMMA_W, 'n', &semaphore[4]};

  for (int i = 0; i < PHIL_T; i++)
    {
      int err =
	pthread_create (&(philoThreads[i]), NULL, &philoThread,
			&(input_v[i]));
      if (err != 0)
	{
	  printf ("Error: %d", err);
  ...
  while (!q_flag)
      ...
      else if (keyInput[1] == 'u')
	{
	  ...
	  if (thread_id < PHIL_T && thread_id >= 0)
	    ...
	    {
	      printf ("Something went wrong. Unknown Thread_ID %d",
		      thread_id);
	    }
