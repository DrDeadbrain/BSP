 ...
 */
void
get_weights (int needed_weights, Weights_s * input, Thread_info threadInfo)
{
 ...
 */
void
displayInfoStatus (Thread_info info)
{
  ...
  for (int i = 0; i < PHIL_T; i++)
    {
      ...
      printf ("ID:%d (%d), %c, %c, [%d, %d, %d] ", ID, wNeeded, KEYP, state,
	      w1, w2, w3);
    }
  printf (" Supply: [%d, %d, %d]\n", _weight_S.twoKG, _weight_S.threeKG,
	  _weight_S.fiveKG);
 ...
 */
void
put_weights (Weights_s * userWStack, Thread_info threadInfo)
{
  pthread_mutex_lock (&mutex);
 ...
 */
bool
checkWeight (int totalWeight, Weights_s * from, Weights_s * to)
{
 ...
 */
void
copyWeight (Weights_s * from, Weights_s * to)
{
  (*to).twoKG = (*from).twoKG;
 ...
 */
int
getTotalWeight (Weights_s wStack)
{
